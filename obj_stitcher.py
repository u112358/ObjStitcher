...
class ObjStitcher:
    def __init__(self, object_length_MM, cam_mm_per_pixel_Y, mark_length, frame_width_P, overlap=100):
        ...
        self.buffer = []  # 存放连续帧
        self.mode = "seek_mark"  # 初始为累积模式

        self.problematic_pending = None
        # 新增：存储首次检测到 mark 尾的全局行位置（在累积模式下）
    ...
    def process_frame(self, frame):
        ...
        if self.mode == "seek_mark":
            mark_exists, mark_position = self.seek_mark(
                self._buffer_to_array(), self.frame_width_P, self.mark_length, visiable=__DEBUG__)
            logger.debug(
                f"seek mark mode ：本帧检测到mark: {mark_exists}, 位置: {mark_position}")
            if mark_exists:
                if total_rows >= mark_position + self.overlap:
                    obj = self._extract_rows(0, mark_position+self.overlap)
                    self._remove_rows(mark_position-self.overlap)
            ...
            if total_rows >= self.block_size:
                ...
                if mark_exists:
                    ...
                    self.problematic_pending = block
...
if __name__ == "__main__":
    ...
    obj_stitcher = ObjStitcher(
        object_length_MM, cam_mm_per_pixel_Y, mark_length, frame_width_P, overlap)
    from frame_capturer import FrameCapturer
    frame_capture = FrameCapturer(mode='local', file_path=r"D:\data\1")
    out_path = r"./out"
